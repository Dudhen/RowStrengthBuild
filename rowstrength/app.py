...
def S_BTN():
    return Pack(padding_top=10, padding_bottom=10, padding_left=12, padding_right=12, flex=1)
...
class RowStrengthApp(toga.App):
    def __init__(self, *args, **kwargs):
        ...
        # Авторитетное время (обход бага Selection.value на iOS)
        self._min_value = "06"
        self._sec_value = "00"
        self._cen_value = "0"
        ...
        # Деревья для сплэша/основного экрана
        self._splash_root = None
        self._root = None

    # --- Синхронизация минут из нативного UITextField (устраняет кейс 16e) ---
    def _sync_min_from_native(self):
        if sys.platform != "ios":
            return
        try:
            native = self.min_sel._impl.native
            if native and native.text:
                txt = str(native.text).strip()
                vals = [row.value for row in list(self.min_sel.items)] or []
                if txt in vals:
                    self._min_value = txt
        except Exception:
            pass
    ...
    def startup(self):
        ...
        # Подготовка под сплэшем и затем показ «готового» UI
    ...
    def _prepare_ui_during_splash(self):
        try:
            ...
            # 3) Временно подменяем контент на основной — чтобы создать native-вью
            prev = self.main_window.content
            self.main_window.content = self._root

            # 4) Полная последовательность «как в рабочем коде»
            ...
            # Принудительный рефреш, чтобы снять «левый край»
            try:
                self._deep_refresh(self._root)
            except Exception:
                pass
            _force_layout_ios(self.main_window)

            # 5) Возврат сплэша обратно
    ...
    def _swap_to_main_ready(self):
        try:
            ...
            # Нудж, который убирает «серую плашку» и «липкий к левому краю» старт
            self._nudge_scrollcontainers()
            try:
                # re-select текущую вкладку, чтобы OptionContainer правильно пересчитал отступы
                idx = self._current_tab_index()
                self._select_tab_index(idx)
                self.tabs.refresh()
            except Exception:
                pass

            # Синхронизация минут из реального UITextField на всякий случай
            self._sync_min_from_native()

            # Финальные пинки лэйаута и клавиатур
            try:
    ...
    def _create_ui_tree(self):
        ...
        self.sec_sel = toga.Selection(items=[_two(i) for i in range(60)], value="00",
    ...
    # ---- Очистка контейнеров результатов ----
    ...
    # ---- Нудж ScrollContainer (чинит стартовые артефакты на iOS) ----
    ...
    # ---- Жёсткая установка "минуты = второй элемент" ----
    ...
    def calculate_erg(self, widget):
        self._dismiss_ios_inputs()
        # Страховка: берем реально отображаемую минуту из native (iOS)
        self._sync_min_from_native()
